                ...
                if let MessageContent::ToolRequest(tool_req) = content {
                    if let Ok(tool_call) = tool_req.tool_call.as_ref() {
                        ...
                        if let Ok(args) = serde_json::from_value::<Value>(tool_call.arguments.clone()) {
                            ...
                            command.is_some_and(|cmd| cmd.contains("git clone") && cmd.contains("michaelneale/mcp-read-pdf"))
                        } else {
                            false
                        }
                    } else {
                    ...
                    if let Ok(tool_call) = tool_req.tool_call.as_ref() {
                        ...
                        if let Ok(args) = serde_json::from_value::<Value>(tool_call.arguments.clone()) {
                            ...
                            command.is_some_and(|cmd| 
                                (cmd.contains("ls") || cmd.contains("find") || cmd.contains("rg")) && 
                ...
                if let MessageContent::ToolRequest(tool_req) = content {
                    if let Ok(tool_call) = tool_req.tool_call.as_ref() {
                        ...
                        if let Ok(args) = serde_json::from_value::<Value>(tool_call.arguments.clone()) {
                            ...
                            command == Some("write") && 
                            path.is_some_and(|p| p.contains("test") || p.ends_with(".py") || p.ends_with(".js") || p.ends_with(".ts")) &&
                            file_text.is_some_and(|text| 
                                (text.contains("test") || text.contains("assert") || 
                                 text.contains("expect") || text.contains("should"))
                            )
                        } else {
                            false
                        }
                ...
                if let MessageContent::ToolRequest(tool_req) = content {
                    if let Ok(tool_call) = tool_req.tool_call.as_ref() {
                        ...
                        if let Ok(args) = serde_json::from_value::<Value>(tool_call.arguments.clone()) {
                            let command = args.get("command").and_then(Value::as_str);
                            command.is_some_and(|cmd| 
                                (cmd.contains("test") || cmd.contains("pytest") || 
                                 cmd.contains("jest") || cmd.contains("mocha") ||
